 ...
 */
package pe.edu.sis.gestfamilia.mysql;
...
public class FamiliaImpl implements FamiliaDAO {
    ...
    public int insertar (Familia familia){
        ...
        DbManager.getInstance().ejecutarProcedimiento("INSERTAR_FAMILIA",in,out);
    ...
    @Override
    public Familia obtener_por_id(int pos) {
        Familia fam=null;
        Map<Integer, Object> in= new HashMap<>();
        in.put(1,pos);
    ...
    public ArrayList<Familia> listarTodos() {
        ...
        try{
            while(rs.next()){
                if(familia==null)familia=new ArrayList<>();
