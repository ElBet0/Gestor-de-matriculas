...
public class ReportesWS {
    ...
    public byte[] generarReporteDeudasFamilia(@WebParam(name = "idFamilia") int idFamilia) {
        ...
        try{
            ...
            URL rutaURLImagen = getClass().getResource("/pe/edu/sis/images/LogoColegio.png");
    ...
    @WebMethod(operationName = "generarReporteMatriculasPeriodo")
    public byte[] generarReporteMatriculasPeriodo(@WebParam(name = "idPeriodo") int idPeriodo,@WebParam(name = "nombrePeriodo") String nombrePeriodo) {
        byte[] reporte = null;
        try{
            //Referenciamos el archivo Jasper
            JasperReport jr = (JasperReport) JRLoader.loadObject(getClass().getResourceAsStream("/pe/edu/sis/reports/ReporteMatriculasPeriodo.jasper"));
            ...
            //Establecemos los parametros que necesita el reporte
            HashMap hm = new HashMap();
            hm.put("PERIODO_NOMBRE", nombrePeriodo);
            hm.put("PERIODO_ID", idPeriodo);
            hm.put("LOGO", imagen);
