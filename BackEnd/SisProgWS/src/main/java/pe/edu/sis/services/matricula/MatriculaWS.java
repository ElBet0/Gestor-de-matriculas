...
public class MatriculaWS {
    ...
    public ArrayList<Matricula> listarMatriculasTodas() {
        System.out.println("ListarMatriculas");
    ...
    public int registrarMatriculaConVacantes(
            ...
            @WebParam(name = "costo_matricula") double costo_matricula,
            @WebParam(name = "costo_pension") double costo_pension) {

        int periodoAulaId = 0;
        try {
            periodoAulaId = 
                    boMatricula.registrarMatriculaConVacantes(alumno_id, 
                            aula_id, costo_matricula, costo_pension);
        } catch (Exception ex) {
            System.out.println(ex.getMessage());
        }
